 ...
 *		&lt;/init-param>
 *		&lt;init-param>
 *			&lt;param-name>mustRests&lt;/param-name>
 *			&lt;param-value>/scripts/jquery-roundmenu/flash/menu.xml&lt;/param-value>
 *		&lt;/init-param>
 *		&lt;init-param>
...
public class RestUrlRewriteFilter extends OncePerRequestFilter implements Filter{
	...
	private String[] mustRests = new String[0];
	...
	private void initParameter(FilterConfig filterConfig) throws IOException {
		...
		String mustRestsString = getStringParameter(filterConfig, "mustRests", null);
		if(StringUtils.hasText(mustRestsString)) {
			mustRests = mustRestsString.split(",");
		}
	...
	private boolean rewriteURL(String from) {
		for(String mustRest : mustRests){
			if(from.startsWith(mustRest)){
				return true;
			}
		}
