...
<script>
	$(function(){
		$('#list').datagrid({
			...
			remoteSort: false,
			pageList:[10],
			url:'${base}/exam/examinee/score_data.json?id=${id}',
			idField:'id',
			frozenColumns:[[
		                    {field:'id',checkbox:true}
		                ]],
			columns:[[
						{field:'title',title:'考试描述',width:200},
						{field:'full_score',title:'总分数',width:50},
						{field:'exam_long_time',title:'考试时长',width:100,
	...
	function dialogScore(scoreId){
    	window.showModalDialog("${base}/exam/examinee/score_detail${ext}?id="+scoreId+"&temp="+new Date().getTime(),null,"dialogWidth=800px;dialogHeight=600px");
    }
