...
import org.paramecium.aop.asm.Attribute;
...
public class ReflectUtils {
	...
	private static final ClassLoader defaultLoader = ReflectUtils.class
			.getClassLoader();
	...
	public static Type[] getExceptionTypes(Member member) {
		if (member instanceof Method) {
			...
			return TypeUtils.getTypes(((Constructor) member)
					.getExceptionTypes());
		} else {
	...
	public static Signature getSignature(Member member) {
		if (member instanceof Method) {
			return new Signature(member.getName(), Type
					.getMethodDescriptor((Method) member));
		} else if (member instanceof Constructor) {
			Type[] types = TypeUtils.getTypes(((Constructor) member)
					.getParameterTypes());
			return new Signature(Constants.CONSTRUCTOR_NAME, Type
					.getMethodDescriptor(Type.VOID_TYPE, types));
	...
	public static Constructor findConstructor(String desc, ClassLoader loader) {
		try {
			...
			return getClass(className, loader).getConstructor(
					parseTypes(desc, loader));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public static Method findMethod(String desc, ClassLoader loader) {
		try {
			...
			return getClass(className, loader).getDeclaredMethod(methodName,
					parseTypes(desc, loader));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static Class getClass(String className, ClassLoader loader,
			String[] packages) throws ClassNotFoundException {
		...
		StringBuffer brackets = new StringBuffer(className.length()
				- dimensions);
		...
		for (int i = 0; i < packages.length; i++) {
			try {
				return Class.forName(prefix + packages[i] + '.' + className
						+ suffix, false, loader);
			} catch (ClassNotFoundException ignore) {
			}
		}
	...
	public static Method findInterfaceMethod(Class iface) {
		...
		if (methods.length != 1) {
			throw new IllegalArgumentException("expecting exactly 1 method in "
					+ iface);
		}
		return methods[0];
	}
	...
	public static Class defineClass(String className, byte[] b,
		...
		Object[] args = new Object[] { className, b, new Integer(0),
				new Integer(b.length), PROTECTION_DOMAIN };
	...
	public static ClassInfo getClassInfo(final Class clazz) {
		...
		final Type sc = (clazz.getSuperclass() == null) ? null : Type
	...
	public static Method[] findMethods(String[] namesAndDescriptors,
		...
		for (int i = 0; i < methods.length; i++) {
			...
			map
					.put(method.getName() + Type.getMethodDescriptor(method),
							method);
		}
		Method[] result = new Method[namesAndDescriptors.length / 2];
		for (int i = 0; i < result.length; i++) {
			result[i] = (Method) map.get(namesAndDescriptors[i * 2]
					+ namesAndDescriptors[i * 2 + 1]);
			if (result[i] == null) {
			}
		}
		return result;
