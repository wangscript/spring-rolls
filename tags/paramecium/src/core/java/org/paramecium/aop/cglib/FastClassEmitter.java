...
class FastClassEmitter extends ClassEmitter {
	...
	private static final Signature SIGNATURE_GET_INDEX = new Signature(
			"getIndex", Type.INT_TYPE, new Type[] { Constants.TYPE_SIGNATURE });
	...
	private static final Type FAST_CLASS = TypeUtils
			.parseType("org.paramecium.aop.cglib.FastClass");
	...
	public FastClassEmitter(ClassVisitor v, String className, Class type) {
		...
		begin_class(Constants.V1_2, Constants.ACC_PUBLIC, className,
				FAST_CLASS, null, Constants.SOURCE_FILE);
		...
		List constructors = new ArrayList(Arrays.asList(type
		...
		e = begin_method(Constants.ACC_PUBLIC, NEW_INSTANCE,
				INVOCATION_TARGET_EXCEPTION_ARRAY);
		e.new_instance(base);
		e.dup();
		e.load_arg(0);
	...
	private void emitIndexBySignature(List methods) {
		CodeEmitter e = begin_method(Constants.ACC_PUBLIC, SIGNATURE_GET_INDEX,
				null);
	...
	private void emitIndexByClassArray(List methods) {
		CodeEmitter e = begin_method(Constants.ACC_PUBLIC, METHOD_GET_INDEX,
				null);
		if (methods.size() > TOO_MANY_METHODS) {
			// hack for big classes
			List signatures = CollectionUtils.transform(methods,
					new Transformer() {
						public Object transform(Object obj) {
							String s = ReflectUtils.getSignature((Method) obj)
									.toString();
			...
			e.load_args();
			List info = CollectionUtils.transform(methods,
					MethodInfoTransformer.getInstance());
	...
	private void signatureSwitchHelper(final CodeEmitter e,
		...
		EmitUtils.string_switch(e, (String[]) signatures
				.toArray(new String[signatures.size()]),
				Constants.SWITCH_STYLE_HASH, callback);
	}

	private static void invokeSwitchHelper(final CodeEmitter e, List members,
			final int arg, final Type base) {
		final List info = CollectionUtils.transform(members,
				MethodInfoTransformer.getInstance());
