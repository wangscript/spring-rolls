...
import org.dily.cache.Cache;
import org.dily.cache.CacheManager;
import org.dily.commons.BeanUtils;
import org.dily.log.Log;
import org.dily.log.LoggerFactory;
import org.dily.orm.annotation.Column;
import org.dily.orm.annotation.Entity;
import org.dily.orm.annotation.Lazy;
import org.dily.orm.annotation.NotUpdate;
import org.dily.orm.annotation.PrimaryKey;
import org.dily.orm.annotation.PrimaryKey.AUTO_GENERATE_MODE;
import org.dily.orm.annotation.ReferenceColumn;
import org.dily.orm.annotation.VirtualColumn;
/**
 ...
 * <br>包及类名(Package Class): <b>org.dily.orm.EntitySqlParser.java</b>
 */
public class EntitySqlParser {
	...
	private final static Cache<String,String> sqlCache = (Cache<String, String>) CacheManager.getDefaultCache("SQL_PARSER", 256, 3600l);
			...
			for(Field field : fields){
				...
				try {
					...
					if(primaryKey!=null&&primaryKey.autoGenerateMode()==AUTO_GENERATE_MODE.MANUAL){
						...
						generateMode = 1;
					}else if(primaryKey!=null&&primaryKey.autoGenerateMode()==AUTO_GENERATE_MODE.DILY_SEQUENCE){
						if(column!=null&&!column.value().isEmpty()){
							columns.add(column.value());
						}else{
							columns.add(BeanUtils.getDbFieldName(field.getName()));
						}
						propertys.add("#DILY_SEQUENCE#");
						generateMode = 3;
	...
	public static String getUpdateSqlNotNull(Object bean){
		...
		for (Class<?> superClass = clazz; superClass != Object.class; superClass = superClass.getSuperclass()) {
			...
			for(Field field : fields){
				...
				field.setAccessible(true);
				try {
					if(BeanUtils.getFieldValue(bean, superClass,field.getName(), field.getType()) == null){
						continue;
					}
					NotUpdate notUpdate = field.getAnnotation(NotUpdate.class);
					if(notUpdate!=null){
