...
class CallbackInfo {
	...
	private static final CallbackInfo[] CALLBACKS = {
			...
			new CallbackInfo(MethodInterceptor.class,
					MethodInterceptorGenerator.INSTANCE),
			new CallbackInfo(InvocationHandler.class,
					InvocationHandlerGenerator.INSTANCE),
			...
			new CallbackInfo(ProxyRefDispatcher.class,
	...
	private static Type determineType(Class callbackType) {
		...
		for (int i = 0; i < CALLBACKS.length; i++) {
			...
			if (info.cls.isAssignableFrom(callbackType)) {
				if (cur != null) {
					throw new IllegalStateException("Callback implements both "
							+ cur + " and " + info.cls);
				}
				cur = info.cls;
			}
