...
class MethodInterceptorGenerator implements CallbackGenerator {
	...
	private static final Type ABSTRACT_METHOD_ERROR = TypeUtils
			.parseType("AbstractMethodError");
	private static final Type METHOD = TypeUtils
			.parseType("java.lang.reflect.Method");
	private static final Type REFLECT_UTILS = TypeUtils
			.parseType("org.paramecium.aop.cglib.ReflectUtils");
	private static final Type METHOD_PROXY = TypeUtils
			.parseType("org.paramecium.aop.cglib.MethodProxy");
	private static final Type METHOD_INTERCEPTOR = TypeUtils
			.parseType("org.paramecium.aop.cglib.MethodInterceptor");
	...
	private static final Signature MAKE_PROXY = new Signature("create",
			METHOD_PROXY, new Type[] { Constants.TYPE_CLASS,
					Constants.TYPE_CLASS, Constants.TYPE_STRING,
					Constants.TYPE_STRING, Constants.TYPE_STRING });
	private static final Signature INTERCEPT = new Signature("intercept",
			Constants.TYPE_OBJECT, new Type[] { Constants.TYPE_OBJECT, METHOD,
					Constants.TYPE_OBJECT_ARRAY, METHOD_PROXY });
	private static final Signature FIND_PROXY = new Signature(FIND_PROXY_NAME,
			METHOD_PROXY, new Type[] { Constants.TYPE_SIGNATURE });
	...
	private static final Signature CSTRUCT_SIGNATURE = TypeUtils
	...
	public void generate(ClassEmitter ce, Context context, List methods) {
		...
		for (Iterator it = methods.iterator(); it.hasNext();) {
			...
			ce.declare_field(Constants.PRIVATE_FINAL_STATIC, methodField,
					METHOD, null);
			ce.declare_field(Constants.PRIVATE_FINAL_STATIC, methodProxyField,
					METHOD_PROXY, null);
			ce.declare_field(Constants.PRIVATE_FINAL_STATIC, EMPTY_ARGS_NAME,
					Constants.TYPE_OBJECT_ARRAY, null);
			CodeEmitter e;

			// access method
			e = ce.begin_method(Constants.ACC_FINAL, impl, method
					.getExceptionTypes());
	...
	private static void superHelper(CodeEmitter e, MethodInfo method) {
		if (TypeUtils.isAbstract(method.getModifiers())) {
			e.throw_exception(ABSTRACT_METHOD_ERROR, method.toString()
					+ " is abstract");
		} else {
			e.load_this();
			e.load_args();
	...
	public void generateFindProxy(ClassEmitter ce, final Map sigMap) {
		final CodeEmitter e = ce.begin_method(Constants.ACC_PUBLIC
				| Constants.ACC_STATIC, FIND_PROXY, null);
		e.load_arg(0);
		...
		EmitUtils.string_switch(e, (String[]) sigMap.keySet().toArray(
				new String[0]), Constants.SWITCH_STYLE_HASH, callback);
		e.end_method();
	}
}
