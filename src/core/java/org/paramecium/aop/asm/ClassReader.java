...
public class ClassReader {
	...
	public void accept(final ClassVisitor classVisitor,
				...
				if (v != 0) {
					...
					for (; j > 0; --j) {
						v = readAnnotationValues(v + 2, c, true, classVisitor
								.visitAnnotation(readUTF8(v, c), i != 0));
					}
				}
			}
		}
		...
		for (; i > 0; --i) {
			...
			FieldVisitor fv = classVisitor.visitField(access, name, desc,
					signature, fieldValueItem == 0 ? null : readConst(
							fieldValueItem, c));
			if (fv != null) {
				if (ANNOTATIONS) {
					for (j = 1; j >= 0; --j) {
			...
			if (mv != null && v != 0) {
				...
				for (; j > 0; --j) {
					...
					if (type == 0) {
						...
						mv.visitTryCatchBlock(start, end, handler, readUTF8(
								items[type], c));
					}
					v += 8;
				}
				...
				if (!skipDebug && varTable != 0) {
					...
					for (; k > 0; --k) {
						...
						mv.visitLocalVariable(readUTF8(w + 4, c), readUTF8(
								w + 6, c), vsignature, labels[start],
	...
	private int readAnnotationValue(int v, final char[] buf, final String name,
		...
		case 'c': // class_info
			...
			break;
		case '@': // annotation_value
			v = readAnnotationValues(v + 2, buf, true, av.visitAnnotation(name,
					readUTF8(v, buf)));
			break;
		case '[': // array_value
			...
			v += 2;
			if (size == 0) {
				return readAnnotationValues(v - 2, buf, false, av
						.visitArray(name));
			}
