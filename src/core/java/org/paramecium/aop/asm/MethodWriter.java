...
class MethodWriter implements MethodVisitor {
	...
	public void visitMethodInsn(final int opcode, final String owner,
		...
		Item i = (opcode == Opcodes.INVOKEDYNAMIC) ? cw.newNameTypeItem(name,
				desc) : cw.newMethodItem(owner, name, desc, itf);
	...
	public void visitLocalVariable(final String name, final String desc,
		...
		if (signature != null) {
			...
			++localVarTypeCount;
			localVarType.putShort(start.position).putShort(
					end.position - start.position).putShort(cw.newUTF8(name))
		...
		++localVarCount;
		localVar.putShort(start.position).putShort(
				end.position - start.position).putShort(cw.newUTF8(name))
	...
	final void put(final ByteVector out) {
		...
		if (ClassReader.SIGNATURES && signature != null) {
			out.putShort(cw.newUTF8("Signature")).putInt(2).putShort(
					cw.newUTF8(signature));
		}
		if (ClassReader.ANNOTATIONS && annd != null) {
	...
	private void resizeInstructions() {
		...
		if (lineNumber != null) {
			...
			while (u < lineNumber.length) {
				writeShort(b, u, getNewOffset(allIndexes, allSizes, 0,
