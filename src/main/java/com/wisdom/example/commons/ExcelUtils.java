...
public class ExcelUtils{
	...
	public static String[] writeExcel(Map<String,String> titles,List values,String savePath,boolean isSequence,int maxResults,String[] mergeFields){
		Assert.notNull(titles,"Error:titles is NULL");
		Assert.notEmpty(titles,"Error:titles is Empty");
		Assert.notNull(values,"Error:values is NULL");
		Assert.notEmpty(values,"Error:values is Empty");
		Assert.notNull(savePath,"Error:savePath is NULL");
		Assert.hasText(savePath,"Error:savePath not's TEXT TYPE");
		...
		String fileName=DateUtils.getCurrentDateTime().toLocaleString().replace(" ",",").replace(":","'");
	...
	private synchronized static void creatExcel(Map<String,String> titles,List<Object> values ,String fileName,boolean isSequence,String[] mergeFields){
		...
		try{
	        ...
	        logger.info("Create Excel SUCCESS:"+fileName);
		}catch (Exception e) {
	...
	private synchronized static void addContext(WritableSheet wsheet,Collection<String> keys,Object object,int count,boolean isSequence,String[] mergeFields)throws Exception{
		...
		for(String fieldName:keys){//循环创建标题
			obj=BeanUtils.getFieldValue(object, fieldName);
			String value="";
			if(obj!=null){
				value=obj.toString();
			}
