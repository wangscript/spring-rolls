...
public abstract class ContentUtils {
	...
	public static float compareSoc(String oriStr, String newStr, float hanzi,float zimu, float fuhao, float shuzi) {
		...
		for (int i = 1; i < matchLength; i++) {
			if ((match[i].x - ori != 1) || (match[i].y - use != 1)) {
				...
				for (; ori + 1 < match[i].x; ori++) {
					float record = getMatchRecord(oriStr.charAt(ori),
							compareList, hanzi);
					result += record;
				}

				for (; use + 1 < match[i].y; use++) {
					float record = getMatchRecord(newStr.charAt(use),
							compareList, hanzi);
					result += record;
				}
		...
		int smallOne = lastOriLength > lastUseLength ? lastUseLength
				: lastOriLength;
		for (int i = 0; i < smallOne; i++) {
			float record = getMatchRecord(oriStr.charAt(ori + i), compareList,
					hanzi);
			result += record;
		}

		for (int i = 0; i < lastOriLength - smallOne; i++) {
			float record = getMatchRecord(oriStr.charAt(ori + smallOne + i),
					compareList, hanzi);
			result += record;
		}

		for (int i = 0; i < lastUseLength - smallOne; i++) {
			float record = getMatchRecord(newStr.charAt(use + smallOne + i),
					compareList, hanzi);
			result += record;
		}
	...
	public static float getContentSoc(String content, float hanzi, float zimu,float fuhao, float shuzi) {
		float result = 0.0F;
	...
	private static float getMatchRecord(char c, List<RecordVO> compareList,
			float defaultRecord) {
